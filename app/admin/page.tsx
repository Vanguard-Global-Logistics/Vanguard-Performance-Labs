  ...
  async function load(value = token) {
    ...
    try {
      ...
      if (!ordersResponse.ok || !statusResponse.ok) {
        setErr(ordersResponse.status === 401 || statusResponse.status === 401 ? "Invalid token, or ADMIN_TOKEN is not configured." : "The admin board could not be loaded.");
        return;
      }
  ...
  async function act(id: string, action: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setErr(data.error ?? "The order action failed.");
        return;
      }
      await load();
    } finally {
      ...
      <div className="launch-page">
        <section className="mx-auto mt-10 max-w-lg">
          <GlassCard className="p-7">
            ...
            <p className="mt-3 text-sm leading-relaxed text-muted">Enter the private ADMIN_TOKEN stored in Vercel. The token is used only for this session and is never displayed.</p>
            <input
              type="password"
              value={token}
            ...
            />
            {err && <p className="mt-3 text-xs text-vanguard-rose" role="alert">{err}</p>}
    ...
    <div className="launch-page admin-page">
      <section className="launch-hero">
        <div className="launch-hero__copy">
          ...
          <p>Confirm payment, release fulfillment, monitor live integrations, and catch missing production settings before they affect a customer.</p>
        </div>
      ...
      <section className="mt-5">
        <GlassCard className={`p-5 ${status?.readyForLiveOrders ? "border-vanguard-teal/35" : "border-vanguard-amber/35"}`}>
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button onClick={() => load()} className="inline-flex items-center gap-2 text-xs text-vanguard-violet"><RefreshCw size={14} /> Refresh status</button>
          </div>
      ...
      <section className="mt-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => load()} className="inline-flex items-center gap-2 text-xs text-vanguard-violet"><RefreshCw size={13} /> Refresh orders</button>
          </div>
        </div>
            ...
            <GlassCard key={order.id} className="p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {order.status === "pending_payment" && (
                  <>
                    <button disabled={busyId === order.id} onClick={() => act(order.id, "confirm_payment")} className="rounded-lg bg-[linear-gradient(135deg,#f1d28a,#d39b3c)] px-3 py-2 text-xs font-bold text-[#130f08] disabled:opacity-50">{busyId === order.id ? "Working…" : "Confirm payment and release"}</button>
                    <button disabled={busyId === order.id} onClick={() => act(order.id, "cancel")} className="rounded-lg border border-vanguard-rose/40 px-3 py-2 text-xs font-bold text-vanguard-rose disabled:opacity-50">Cancel</button>
                  </>
                )}
                {order.status === "payment_confirmed" && order.fulfillment === "ship" && (
                  <button disabled={busyId === order.id} onClick={() => act(order.id, "mark_shipped")} className="rounded-lg border border-vanguard-violet/40 px-3 py-2 text-xs font-bold text-vanguard-violet disabled:opacity-50">Mark shipped</button>
                )}
                {(order.status === "payment_confirmed" || order.status === "shipped") && (
                  <button disabled={busyId === order.id} onClick={() => act(order.id, "complete")} className="rounded-lg border border-white/15 px-3 py-2 text-xs font-bold text-bone disabled:opacity-50">Complete</button>
                )}
              </div>
            </GlassCard>
          ))}
      ...
      <div className="mt-6 flex items-start gap-2 text-[10px] leading-relaxed text-muted"><ShieldCheck size={15} className="mt-0.5 shrink-0 text-vanguard-amber" /> This board is excluded from search indexing and requires the private server-side ADMIN_TOKEN for every read or write action.</div>
    </div>
  );
}
